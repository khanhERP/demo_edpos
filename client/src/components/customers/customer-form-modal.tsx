...
export function CustomerFormModal({ isOpen, onClose, customer, initialPhone }: CustomerFormModalProps) {
  ...
  const { data: orderHistory, isLoading: ordersLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await apiRequest("GET", `https://ae5ea441-9a81-4f0c-badc-1b445a58a294-00-bx7jg4f6rly0.sisko.replit.dev/api/orders`);
  ...
  const onSubmit = async (data: CustomerFormData) => {
    ...
    if (!customer?.id && (!customerIdToUse || customerIdToUse === t('common.autoGenerated'))) {
      ...
      try {
        const response = await fetch('https://ae5ea441-9a81-4f0c-badc-1b445a58a294-00-bx7jg4f6rly0.sisko.replit.dev/api/customers/next-id');
